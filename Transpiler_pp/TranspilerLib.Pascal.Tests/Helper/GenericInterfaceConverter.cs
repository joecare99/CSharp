    ...
    public override TInterface? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        foreach (var jsonProp in root.EnumerateObject())
        {
            ...
            try
            {
                ...
                {
                    // Bei CodeBlock-Typen: Direktes Setzen von Parent fügt das Kind automatisch der SubBlocks-Liste hinzu.
                    // list.Add(child) würde bei ParentedItemsList zu doppelten Einträgen führen.
                    foreach (var el in jsonProp.Value.EnumerateArray())
                    {
                        var child = JsonSerializer.Deserialize<ICodeBlock>(el.GetRawText(), options);
                        if (child != null)
                        {
                            child.Parent = (ICodeBlock)instance;
                        }
                    }
                    continue;
                }
