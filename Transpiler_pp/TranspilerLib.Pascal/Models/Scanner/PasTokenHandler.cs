...
{
    /*
        Pseudocode / Änderungsplan:
        - Enum PasTokenState um InHexNumber erweitern.
        - Dictionary _tokenStateHandler um Handler für InHexNumber ergänzen.
        - Neue Methode HandleHexNumber:
            * Nächstes Zeichen prüfen: solange Hex-Zeichen (0-9, A-F, a-f) -> weiter.
            * Wenn nächstes Zeichen kein Hex-Zeichen -> EmitToken(Number).
        - Hilfsmethode IsHexDigit(char c) hinzufügen.
        - In HandleDefault neuen case:
              case '$' when IsHexDigit(GetNxtChar(...)):
                  setze Pos2, State = InHexNumber.
                  break;
          Falls nach '$' kein Hex-Zeichen: Default-Operation greift (wird als Operation emittiert).
    */
    ...
    private static readonly Dictionary<Enum, Action<TokenDelegate?, string, TokenizeData>> _tokenStateHandler = new()
    {
        ...
        {PasTokenState.InHexNumber, HandleHexNumber},
    };
    ...
    private static void HandleNumber(TokenDelegate? token, string code, TokenizeData data)
    {
        ...
        if (code[data.Pos] == '.' && nxtChar == '.')
        {
            EmitToken(token, data, CodeBlockType.Number, code);
            data.Pos2 = data.Pos;
            data.State = 0;
        }
    }

    private static void HandleHexNumber(TokenDelegate? token, string code, TokenizeData data)
    {
        var nxtChar = GetNxtChar(data.Pos, code);
    ...
    private static void HandleDefault(TokenDelegate? token, string code, TokenizeData data)
    {
        switch (code[data.Pos])
        {
            ...
            case '$':
                // Start Hex-Zahl
                data.Pos2 = data.Pos;
                data.State = (int)PasTokenState.InHexNumber;
                data.Pos--;
                break;
            case ' ':
            case '\t':
