    ...
    public static IEnumerable<object[]> TestListParse2 => new object[][]
{
    ...
    {
        ["1", new[]  { Encoding.UTF8.GetBytes(TestCSDataClass.test1Data),  Encoding.UTF8.GetBytes(TestCSDataClass.cExp1Log  )}],
        ["2", new[]  { Encoding.UTF8.GetBytes(TestCSDataClass.test2Data),  Encoding.UTF8.GetBytes(TestCSDataClass.cExp2Log  )}],
        ["8", new[]  { Encoding.UTF8.GetBytes(TestCSDataClass.test8Data),  Encoding.UTF8.GetBytes(TestCSDataClass.cExp8Log  )}],
        ["9", new[]  { Encoding.UTF8.GetBytes(TestCSDataClass.test9Data),  Encoding.UTF8.GetBytes(TestCSDataClass.cExp9Log  )}],
        ["10", new[] { Encoding.UTF8.GetBytes(TestCSDataClass.test10Data), Encoding.UTF8.GetBytes(TestCSDataClass.cExp10Log )}],
    ...
    public void Tokenize2Test(string _, object[] data, List<TokenData> expList)
    {
        if (data[0] is byte[] bAct)
            _testClass.OriginalCode = Encoding.UTF8.GetString(bAct);
        else
