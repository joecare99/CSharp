...
public class CEventData : CRSDataC<EEventProp, (EEventArt eArt, int iLink, short iLfNr)>, IEventData
{
    ...
    public string sArtText => (_sArtText ??= _GetText?.Invoke(iArtText)) ?? $"{iArtText}";
    ...
    public CEventData(IRecordset dB_EventTable, bool xNoInit = false) : base(dB_EventTable, xNoInit)
    {
    }

    public override void FillData(IRecordset dB_EventTable)
    {
        ReadID(dB_EventTable);
    ...
    {
        dDatumV = dB_EventTable.Fields[EventFields.DatumV].AsDate();
        dDatumB = dB_EventTable.Fields[EventFields.DatumB].AsDate();
    }

    private void FillStringFields(IRecordset dB_EventTable)
    {
        sDeath = dB_EventTable.Fields[EventFields.tot].AsString();
        sBem[1] = dB_EventTable.Fields[EventFields.Bem1].AsString();
        sBem[2] = dB_EventTable.Fields[EventFields.Bem2].AsString();
        sBem[3] = dB_EventTable.Fields[EventFields.Bem3].AsString();
        sBem[4] = dB_EventTable.Fields[EventFields.Bem4].AsString();
        sZusatz = dB_EventTable.Fields[EventFields.Zusatz].AsString();
        sReg = dB_EventTable.Fields[EventFields.Reg].AsString();
        sVChr = dB_EventTable.Fields[EventFields.VChr].AsString();
        sDatumV_S = dB_EventTable.Fields[EventFields.DatumV_S].AsString();
        sDatumB_S = dB_EventTable.Fields[EventFields.DatumB_S].AsString();
        sOrt_S = dB_EventTable.Fields[EventFields.Ort_S].AsString();
    }

    public override void ReadID(IRecordset dB_EventTable)
    {
        eArt = dB_EventTable.Fields[EventFields.Art].AsEnum<EEventArt>();
        iLfNr = dB_EventTable.Fields[EventFields.LfNr].AsInt();
        iPerFamNr = dB_EventTable.Fields[EventFields.PerFamNr].AsInt();
    }

    private void FillIntFields(IRecordset dB_EventTable)
    {
        iPrivacy = dB_EventTable.Fields[EventFields.priv].AsInt();
        iArtText = dB_EventTable.Fields[EventFields.ArtText].AsInt();
        iAn = dB_EventTable.Fields[EventFields.an].AsInt();
        iHausNr = dB_EventTable.Fields[EventFields.Hausnr].AsInt();
        iCausal = dB_EventTable.Fields[EventFields.Causal].AsInt();
        iDatumText = dB_EventTable.Fields[EventFields.DatumText].AsInt();
        iOrt = dB_EventTable.Fields[EventFields.Ort].AsInt();
        iKBem = dB_EventTable.Fields[EventFields.KBem].AsInt();
        iPlatz = dB_EventTable.Fields[EventFields.Platz].AsInt();
        iGrabNr = dB_EventTable.Fields[EventFields.GrabNr].AsInt();
    }

    protected override IRecordset? Seek((EEventArt eArt, int iLink, short iLfNr) iD)
    {
        IRecordset rs = _db_Table;
        if (iD.eArt == rs.Fields[EventFields.Art].AsEnum<EEventArt>()
            && iD.iLink == rs.Fields[EventFields.PerFamNr].AsInt()
            && iD.iLfNr == (short)rs.Fields[EventFields.LfNr].AsInt())
        {
            return rs;
        }
        else
            ...
            foreach (var s in strings)
                switch (s)
                {
                    case nameof(IEventData.sBem):
                        if ((f = rs.Fields[EventFields.Bem1]).AsString() != (d = sBem[1]))
                            SetData(rs, f, d);
                        if ((f = rs.Fields[EventFields.Bem2]).AsString() != (d = sBem[2]))
                            SetData(rs, f, d);
                        if ((f = rs.Fields[EventFields.Bem3]).AsString() != (d = sBem[3]))
                            SetData(rs, f, d);
                        if ((f = rs.Fields[EventFields.Bem4]).AsString() != (d = sBem[4]))
                            SetData(rs, f, d);
                        break;
                }
        else
    ...
    public override void SetDBValues(IRecordset dB_EventTable, Enum[]? asProps = null)
    {
        ...
        foreach (var prop in asProps)
            switch (prop.AsEnum<EEventProp>())
            {
                ...
                case EEventProp.iPerFamNr:
                    dB_EventTable.Fields[EventFields.PerFamNr].Value = iPerFamNr;
                    break;
                case EEventProp.iLfNr:
                    dB_EventTable.Fields[EventFields.LfNr].Value = iLfNr;
                    break;
                case EEventProp.iArtText:
                    dB_EventTable.Fields[EventFields.ArtText].Value = iArtText;
                    break;
                case EEventProp.iPrivacy:
                    dB_EventTable.Fields[EventFields.priv].Value = iPrivacy;
                    break;
                case EEventProp.xIsDead:
                    dB_EventTable.Fields[EventFields.tot].Value = sDeath;
                    break;
                case EEventProp.dDatumV:
                    dB_EventTable.Fields[EventFields.DatumV].Value = dDatumV;
                    break;
                case EEventProp.sReg:
                    dB_EventTable.Fields[EventFields.Reg].Value = sReg;
                    break;
                case EEventProp.sDatumV_S:
                    dB_EventTable.Fields[EventFields.DatumV_S].Value = sDatumV_S;
                    break;
                case EEventProp.sVChr:
                    dB_EventTable.Fields[EventFields.VChr].Value = sVChr;
                    break;
                case EEventProp.dDatumB:
                    dB_EventTable.Fields[EventFields.DatumB].Value = dDatumB;
                    break;
                case EEventProp.sDatumB_S:
                    dB_EventTable.Fields[EventFields.DatumB_S].Value = sDatumB_S;
                    break;
                case EEventProp.sZusatz:
                    dB_EventTable.Fields[EventFields.Zusatz].Value = sZusatz;
                    break;
                case EEventProp.sOrt_S:
                    dB_EventTable.Fields[EventFields.Ort_S].Value = sOrt_S;
                    break;
                case EEventProp.iOrt:
                    dB_EventTable.Fields[EventFields.Ort].Value = iOrt;
                    break;
                case EEventProp.iDatumText:
                    dB_EventTable.Fields[EventFields.DatumText].Value = iDatumText;
                    break;
                case EEventProp.iKBem:
                    dB_EventTable.Fields[EventFields.KBem].Value = iKBem;
                    break;
                case EEventProp.iPlatz:
                    dB_EventTable.Fields[EventFields.Platz].Value = iPlatz;
                    break;
                case EEventProp.iCausal:
                    dB_EventTable.Fields[EventFields.Causal].Value = iCausal;
                    break;
                case EEventProp.iGrabNr:
                    dB_EventTable.Fields[EventFields.GrabNr].Value = iGrabNr;
                    break;
                case EEventProp.iAn:
                    dB_EventTable.Fields[EventFields.an].Value = iAn;
                    break;
                case EEventProp.iHausNr:
                    dB_EventTable.Fields[EventFields.Hausnr].Value = iHausNr;
                    break;
                case EEventProp.sBem:
                    dB_EventTable.Fields[EventFields.Bem1].Value = sBem[1];
                    dB_EventTable.Fields[EventFields.Bem2].Value = sBem[2];
                    dB_EventTable.Fields[EventFields.Bem3].Value = sBem[3];
                    dB_EventTable.Fields[EventFields.Bem4].Value = sBem[4];
                    break;
                default:
