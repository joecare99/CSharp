...
#pragma warning restore CS8618 // Ein Non-Nullable-Feld muss beim Beenden des Konstruktors einen Wert ungleich NULL enthalten. Erwägen Sie die Deklaration als Nullable.
        ...
        {
            testRS.NoMatch.Returns(iActPers is not (> 0 and < 3));
            Assert.AreEqual(xExp, this.Exists(eIx, iActPers));
        ...
        public void ForEachDoTest()
        {
            ...
            var iCnt = 0;
            this.ForEachDo(TestIndex._MyKeyIndex,TestIndexField.ID, 1, ce =>
            {
                Assert.IsNotNull( ce);
        ...
        public void ForEachDo2Test()
        {
            testRS.Fields["ID"].Value.Returns(1, 1, 1, 1, 3);
            testRS.NoMatch.Returns(false);
            this.ForEachDo(TestIndex._MyKeyIndex, TestIndexField.ID, 1, null!);
            Assert.AreEqual("_MyKeyIndex", testRS.Index);
            testRS.Received(0).MoveFirst();
        ...
        public void ForEachDo3Test()
        {
            ...
            this.ForEachDo(TestIndex._MyKeyIndex, TestIndexField.ID, 1, rs =>
            {
                iCnt++;
        ...
        {
            testRS.NoMatch.Returns(iActPers is not (> 0 and < 3));
            Assert.AreEqual(xExp , this.ReadData(iActPers,out var ce));
            if (xExp)
            {
                Assert.IsNotNull(ce);
                Assert.IsInstanceOfType(ce, typeof(ITestData));
        ...
        {
            testRS.NoMatch.Returns(iActPers is not (> 0 and < 3));
            Assert.AreEqual(xExp, this.ReadData(TestIndex._MyKeyIndex, iActPers, out var ce));
            if (xExp)
            {
                Assert.IsNotNull(ce);
                Assert.IsInstanceOfType(ce, typeof(ITestData));
