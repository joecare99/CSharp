...
using static TranspilerLib.Interfaces.Code.ICSCode;

namespace VBUnObfusicator.Models.Scanner;
...
public partial class CSCode : ICSCode
{
    ...
    private ITokenHandler _tokenStateHandler = csTokenHandler;

    public void Tokenize(ICodeBase.TokenDelegate? token)
    {
        TokenizeData data = new();
        while (data.Pos < OriginalCode.Length 
            && _tokenStateHandler.TryGetValue(data.State, out var Handler))
        {
            //Debug:
