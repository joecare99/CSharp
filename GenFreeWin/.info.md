## GenFreeBrowser Map Extraction Snapshot

- Target areas:
  - `GenFreeBrowser.Map`
  - `GenFreeBrowser`
  - `MapDemo`
- Goal: extract the reusable `MapView` and related mapping infrastructure from `GenFreeBrowser` into a separate project while keeping existing consumers functional.
- Result:
  - added the new `GenFreeBrowser.Map` WPF library project
  - moved the complete `GenFreeBrowser\Map` component tree into the new project
  - updated `GenFreeBrowser` and `MapDemo` to reference `GenFreeBrowser.Map`
  - updated `MapDemo` XAML to reference the new `GenFreeBrowser.Map` assembly directly
- Validation:
  - full workspace build succeeded in Visual Studio tooling
