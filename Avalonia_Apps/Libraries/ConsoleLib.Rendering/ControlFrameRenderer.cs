...

{
    ...
    {
        // What has been scrolled out of view
        int scrollX = 0, scrollY = 0;
        if (control is ScrollViewer sv)
        {
            scrollX = Math.Max(0, sv.Offset.X);
            scrollY = Math.Max(0, sv.Offset.Y);
        }
        ...
        foreach (var child in control.Children.Reverse<IControl>())
        {
            ...
            {
                /* Non-fixed: shift canvas coords by scroll offset */
                canvasCol -= scrollX;
                canvasRow -= scrollY;
            ...
            {
                /* Non-scroll container children: standard bounds check */
                int x = child.RealDim.Location.X;
                int y = child.RealDim.Location.Y;
                if (x < 0 || x >= size.Width ||
                    y < 0 || y >= size.Height)
                    continue;
