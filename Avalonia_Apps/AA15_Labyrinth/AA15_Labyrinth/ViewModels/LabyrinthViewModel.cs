...
public interface ILabyrinthViewModel
{
 int Seed { get; set; }
...
public sealed class LabyrinthViewModel : ILabyrinthViewModel
{
 ...
 public double CellSize { get; set; } =12.0;
 public double LineThickness { get; set; } =1.8;
 public double CornerRadius { get; set; } =4.0;
 public Thickness Padding { get; set; } = new Thickness(12);
 ...
 DrawSmoothed(mgc, chain.Select(id => { var (x, y) = FromId(id); return P(x, y); }).ToList(), CornerRadius);
 }
 }
 }
 _maze = mazeGeom;
 ...
 {
 DrawSmoothed(sgc, solIds.Select(id => { var (x, y) = FromId(id); return P(x, y); }).ToList(), CornerRadius);
 }
 _sol = sol;
 }
 else _sol = null;
