...
#pragma warning restore CS8618 // Ein Non-Nullable-Feld muss beim Beenden des Konstruktors einen Wert ungleich NULL enthalten. Erwägen Sie die Deklaration als Nullable.
    ...
    private void CreateView()
    {
        try
        {
            ...
            {
                var dialog = Substitute.For<IDialogWindow>();
                dialog.DataContext.Returns(o);
                dialog.ShowDialog(Arg.Any<Avalonia.Controls.Window>()).Returns(_ =>
                {
                    DoLog($"ShowDialog()=>{xResult}");
                    var dialogViewModel = (DialogWindowViewModel)dialog.DataContext!;
                    dialogViewModel.Name = sNewName;
                    dialogViewModel.Email = sNewEmail;
                    return Task.FromResult((xResult, (dialogViewModel.Name, dialogViewModel.Email)));
                });
                return dialog;
            };

            var windowOpened = typeof(MainWindow).GetMethod("Window_Opened", BindingFlags.Instance | BindingFlags.NonPublic);
            Assert.IsNotNull(windowOpened);
            windowOpened.Invoke(testView, new object?[] { this, EventArgs.Empty });
        }
