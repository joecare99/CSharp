...
/// <summary>
/// Coordinates marble rendering and user interaction for the Avalonia prototype.
/// </summary>
public sealed partial class BoardViewModel : ObservableObject
{
    ...
    public bool HandleArrowKey(Avalonia.Input.Key key, bool shiftPressed)
    {
        (int columnOffset, int rowOffset)? delta = key switch
        {
            Avalonia.Input.Key.Left => (-1, 0),
            Avalonia.Input.Key.Right => (1, 0),
            Avalonia.Input.Key.Up => (0, -1),
            Avalonia.Input.Key.Down => (0, 1),
            _ => null,
        };
