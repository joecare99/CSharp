...
public sealed class MySqlNoticeRepository : INoticeRepository
{
    ...
    {
        var statement = MySqlNoticeSql.BuildFind(filter);
        return await QueryAsync(statement.CommandText, statement.Parameters, cancellationToken).ConfigureAwait(false);
    }
    ...
    public async Task<IReadOnlyList<string>> GetPlaceNamesAsync(CancellationToken cancellationToken = default)
    {
        ...
        var statement = MySqlNoticeSql.BuildPlaceNames();
        using var command = connection.CreateCommand(); command.CommandText = statement.CommandText;
    ...
    {
        var statement = MySqlNoticeSql.BuildLinkCandidates(noticeId);
        return QueryAsync(statement.CommandText, statement.Parameters, cancellationToken);
    }
