...
namespace Calc64Base.Tests
{
    ...
    public class Calc64Tests
    {
        ...
        /// </summary>
        /// <autogeneratedoc />
        private Calc64 testCalc64 = null!;
        ...
        {
            {
                ...
                {
                    SetNeedAccumulator = true,
                    SetNeedRegister = iExpArgCount > 1,
                    SetNeedMemory= iExpArgCount > 2,
                    oExpData = new object[iExpArgCount],
                    oSetData = new object[iExpArgCount],
                    iExpLength = iExpArgCount,
                    xSetResult = xExpResult
                };

                testCalc64.Accumulator = oInData[0];
                if (co.NeedRegister) testCalc64.Register = oInData[1];
                if (co.NeedMemory) testCalc64.Memory = oInData[2];
                for (var i = 0; i < iExpArgCount; i++) {
                    co.oExpData[i] = oInData[i];
                    co.oSetData[i] = oOutData[i];
                }
                Assert.AreEqual(xExpResult, testCalc64.DoOperation(co),Name);
        ...
        public void DoOperationTest1(string Name, string Op, int iExpArgCount, long[] oInData, long[] oOutData, bool xExpResult)
        {
            ...
            Assert.AreEqual(xExpResult, testCalc64.DoOperation(Op), Name);
        ...
        /// <summary>
        /// Determines whether [is register operation test] [the specified name].
        /// </summary>
        /// <param name="Name">The name.</param>
        ...
        [DataRow("1-", 1, "?", false)]
        [DataRow("0-", 0, "?", false)]
        public void IsRegisterOperationTest(string Name, int iOp, string _, bool xExpResult)
        {
            testCO.SetNeedRegister = xExpResult;
            if (iOp==1)
            Assert.AreEqual(xExpResult, Calc64.IsRegisterOperation(testCO), Name);
            else    
            Assert.AreEqual(xExpResult, Calc64.IsRegisterOperation((CalcOperation?)null),Name);
        }

        /// <summary>
        /// Determines whether [is register operation test1] [the specified name].
        /// </summary>
        /// <param name="Name">The name.</param>
        /// <param name="iOp">The i op.</param>
        /// <param name="sOp">The s op.</param>
        ...
        [DataRow("16", 16, "==", true)]
        public void IsRegisterOperationTest1(string Name, int _, string sOp, bool xExpResult)
        {
            testCO.SetNeedRegister = xExpResult;
            Assert.AreEqual(xExpResult, Calc64.IsRegisterOperation(sOp),Name);
        }

        /// <summary>
        /// Determines whether [is register operation test2] [the specified name].
        /// </summary>
        /// <param name="Name">The name.</param>
        /// <param name="iOp">The i op.</param>
        /// <param name="sOp">The s op.</param>
        ...
        [DataRow("15", 15, "x~", true)]
        [DataRow("16", 16, "==", true)]
        public void IsRegisterOperationTest2(string Name, int iOp, string _,bool xExpResult)
        {
            testCO.SetNeedRegister = xExpResult;
            Assert.AreEqual(xExpResult, Calc64.IsRegisterOperation(iOp), Name);
        }
        ...
        public void ToCalcOperationTest(string Name, int iOp, string? sOp)
        {
            ...
            if (iOp != 0)
            {
                Assert.IsNotNull(co = Calc64.ToCalcOperation(sOp),Name);
                Assert.AreEqual(sOp, co.ShortDesc);
            }
            else
                Assert.IsNull(co = Calc64.ToCalcOperation(sOp), Name);
            Assert.AreEqual(iOp, co?.ID ?? 0, $"{Name}.ID");
        }
        ...
        {
            CalcOperation? co;
            Assert.IsNotNull(co=Calc64.ToCalcOperation(iOp), Name);
            Assert.AreEqual(sOp, co.ShortDesc);
