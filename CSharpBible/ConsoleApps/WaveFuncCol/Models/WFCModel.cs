...
namespace WaveFunCollapse.Models;

public class WFCModel : IWFCModel
{
    ...
    private readonly EDirections[] validDir
        = [EDirections.Up, EDirections.Down, EDirections.Left, EDirections.Right];
    ...
    private int GetPColorIndex(Color c, Dictionary<Color, int> gc)
    {
        return gc.Keys.IndexOf(c);
    }

    public IReadOnlyList<TileData>? AnalyseImage(string filename)
    {
        ShowImage1(filename);
        var bitmap = LoadImage(filename);
        if (bitmap == null)
        ...
        for (int j = 0; j < bitmap.Height; j++)
            for (int i = 0; i < bitmap.Width; i++)
            {
                ...
                foreach (var ttile in ttileRelKoor)
                {
                    ...
                    tci.Add(GetPColorIndex(bitmap.GetPixel(tRelData.X, tRelData.Y), gc));
                }
        ...
        ShowTiles(gc, ttileRelKoor, lTile);
        foreach (var t in lTile)
        {
            foreach (var t2 in lTile.Where(t2 => t2.Index >= t.Index))
            {
                if (t.OrgData is Tuple<Point, IList<int>> od11
                    && t2.OrgData is Tuple<Point, IList<int>> od2)
                    foreach (EDirections eD in validDir)
                    {
                        var flag = true;
                        var shift = ttileRelKoor[(int)eD];
                            foreach (var k in ttileRelKoor.Where(k => (k.X + shift.X) is > -2 and < 2 && (k.Y + shift.Y) is > -2 and < 2))
                            {
                                if (od11.Item2[k.X + k.Y * 3] != od11.Item2[(k.X + shift.X) + (k.Y + shift.Y) * 3])
                                {
                                    flag = false;
                                    break;
                                }
                        ...
                        if (flag)
                        {
                            if (!t.Neighbors.ContainsKey(eD))
                                t.Neighbors[eD] = new List<int>();
                            t.Neighbors[eD].Add(t2.Index);
                            var eoD = eD.Opposite(); 
                            if (!t2.Neighbors.ContainsKey(eoD))
                                t2.Neighbors[eoD] = new List<int>();
                            t2.Neighbors[eoD].Add(t2.Index);
                        }
                    };
            }
        }
    ...
    private void ShowTiles(Dictionary<Color, int> gc, List<Point> ttileRelKoor, List<TileData> lTile)
    {
        var s = 0;
        foreach (var t in lTile)
        {
    ...
    public void DisplayTest3(IRandom rnd)
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            {
                byte r = (byte)Math.Min(Math.Max(Math.Round(1.25 + 0.5 * rnd.NextDouble() + Math.Sin((j % D2Width - i * 0.4) * -0.6) * 1.5) * 64, 0), 255);
                byte g = (byte)Math.Min(Math.Max(Math.Round(1.25 + 0.5 * rnd.NextDouble() + Math.Sin((j / D2Width + j % D2Width / 2 + i * 0.4) * 0.6) * 1.5) * 64, 0), 255);
                byte b = (byte)Math.Min(Math.Max(Math.Round(1.25 + 0.5 * rnd.NextDouble() + Math.Sin((j / D2Width - j % D2Width / 2 - i * 0.4) * 0.6) * 1.5) * 64, 0), 255);
