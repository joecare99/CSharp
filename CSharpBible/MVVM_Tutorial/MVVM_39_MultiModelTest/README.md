...
> Coordinating multiple model types.
...
## Build & Run
`
dotnet build MVVM_39_MultiModelTest
`
If multiple target frameworks (e.g., net6.0-windows + net481) exist, you can specify one:
`
dotnet build MVVM_39_MultiModelTest -f net6.0-windows
`
Run (where an executable host exists):
`
dotnet run --project MVVM_39_MultiModelTest -f net6.0-windows
`

## Testing
If a companion test project exists it is listed below. Execute:
`
dotnet test MVVM_39_MultiModelTestTests
`
...
| Metric | Status |
|--------|--------|
| Unit Tests | Implemented (see associated *Tests* project: MVVM_39_MultiModelTestTests) |
| Line Coverage | TBD |
| Branch Coverage | TBD |
| Method Coverage | TBD |
...
`
dotnet test MVVM_39_MultiModelTestTests --framework net481 /p:CollectCoverage=true /p:CoverletOutputFormat=lcov
`

Modern multi-target (.NET >= 6):
`
dotnet test MVVM_39_MultiModelTestTests -c Debug /p:CollectCoverage=true /p:CoverletOutputFormat=lcov \
