...
## Build & Run
`
dotnet build MVVM_28_1_DataGridExt
`
If multiple target frameworks (e.g., net6.0-windows + net481) exist, you can specify one:
`
dotnet build MVVM_28_1_DataGridExt -f net6.0-windows
`
Run (where an executable host exists):
`
dotnet run --project MVVM_28_1_DataGridExt -f net6.0-windows
`

## Testing
If a companion test project exists it is listed below. Execute:
`
dotnet test MVVM_28_1_DataGridExtTests
`
...
| Metric | Status |
|--------|--------|
| Unit Tests | Implemented (see associated *Tests* project: MVVM_28_1_DataGridExtTests) |
| Line Coverage | TBD |
| Branch Coverage | TBD |
| Method Coverage | TBD |
...
`
dotnet test MVVM_28_1_DataGridExtTests --framework net481 /p:CollectCoverage=true /p:CoverletOutputFormat=lcov
`

Modern multi-target (.NET >= 6):
`
dotnet test MVVM_28_1_DataGridExtTests -c Debug /p:CollectCoverage=true /p:CoverletOutputFormat=lcov \
