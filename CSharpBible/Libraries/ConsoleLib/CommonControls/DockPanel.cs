...
public class DockPanel : Control, IGroupControl
{
    ...
    public override IControl Add(IControl control) { var desiredSize = control.size; var r = base.Add(control); if (Pending.TryGetValue(control, out var d)) { _docks[control] = d.Value; Pending.Remove(control); } _sizes[control] = desiredSize; control.OnResize += Changed; control.OnMove += Changed; Arrange(); return r; }
    public override IControl Remove(IControl control) { control.OnResize -= Changed; control.OnMove -= Changed; _sizes.Remove(control); var r = base.Remove(control); Arrange(); return r; }
    ...
    private void Changed(object sender, EventArgs e) { if (_arranging) return; if (sender is IControl control) _sizes[control] = control.size; Arrange(); }
    private void Arrange()
    {
        ...
        try
        {
            ...
            for (var i = 0; i < Children.Count; i++)
            {
                ...
                var d = GetDock(child); var s = _sizes.TryGetValue(child, out var desiredSize) ? desiredSize : child.size;
                Rectangle slot;
                if (fill) slot = area;
                ...
                if (!fill) { if (d == Dock.Left) { area.X += slot.Width; area.Width -= slot.Width; } else if (d == Dock.Right) area.Width -= slot.Width; else if (d == Dock.Top) { area.Y += slot.Height; area.Height -= slot.Height; } else area.Height -= slot.Height; }
            }
        }
        finally
        {
