...
/// <summary>
/// Default widget set that preserves the existing console-backed rendering behavior.
/// </summary>
public sealed class ConsoleWidgetSet : IWidgetSet, IConsoleWidgetHost
{
    ...
    public void WriteTerminalCell(Terminal terminal, int x, int y, char character, System.ConsoleColor foreground, System.ConsoleColor background)
        => ConsoleFramework.Canvas.OutTextXY(terminal.RealDim.Left + 1 + x, terminal.RealDim.Top + 1 + y, character, foreground, background);

    public void ClearTerminalCell(Terminal terminal, int x, int y, char character, System.ConsoleColor foreground, System.ConsoleColor background)
        => ConsoleFramework.Canvas.OutTextXY(terminal.RealDim.Left + 1 + x, terminal.RealDim.Top + 1 + y, character, foreground, background);

    public void FillShadow(Rectangle rectangle, System.ConsoleColor foreground, System.ConsoleColor background, char fillChar)
    ...
    public void DrawControl(IControl control)
    {
        lock (control)
        {
            ...
            var innerWidth = Math.Max(0, control.RealDim.Width - 2);
            var centeredButtonText = buttonText.Length > innerWidth
    ...
    public void DrawPanel(IGroupControl panel)
    {
        ...
        {
            var border = ResolveBorder(hasBorder.BorderDefinition);
    ...
    /// <inheritdoc/>
    public void RedrawPanel(IGroupControl _panel, Rectangle dimension)
    {
        if (!(_panel is Panel panel)) return;

        var border = ResolveBorder(panel.BorderDefinition);
    ...
    /// <inheritdoc/>
    public void DrawMenuItem(IControl _menuItem)
    {
        if (!(_menuItem is MenuItem menuItem)) return;
    ...
    /// <inheritdoc/>
    public void DrawMenuBar(IGroupControl _menuBar)
    {
        if (!(_menuBar is MenuBar menuBar)) return;
    ...
    /// <inheritdoc/>
    public void DrawListBox(IControl _listBox)
    {
        if (!(_listBox is ListBox listBox)) return;
    ...
    /// <inheritdoc/>
    public void DrawScrollBar(IControl _scrollBar)
    {
        if (!(_scrollBar is ScrollBar scrollBar)) return;
    ...
    /// <inheritdoc/>
    public void DrawTextBox(IControl _textBox)
    {
        if (!(_textBox is TextBox textBox)) return;
    ...
    /// <inheritdoc/>
    public void DrawTerminal(IControl _terminal)
    {
        if (!(_terminal is Terminal terminal)) return;

        var border = ResolveBorder(terminal.BorderDefinition);
    ...
    /// <inheritdoc/>
    public void RedrawTerminal(IControl _terminal, Rectangle dimension)
    {
        if (!(_terminal is Terminal terminal)) return;
