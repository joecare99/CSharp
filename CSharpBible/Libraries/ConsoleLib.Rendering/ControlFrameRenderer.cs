...
public sealed class ControlFrameRenderer : IControlFrameRenderer
{
            ...
            default:
                ...
                if (control is Button button)
                {
                    ...
                    if (button.Enabled)
                    {
                        if (button.Active)
                        {
                            PutCell(cells, size, x, y, '[', foreground, background);
                            PutCell(cells, size, x + width - 1, y, ']', foreground, background);
                        }
                        else
                        {
                            PutCell(cells, size, x, y, '|', foreground, background);
    ...
    private static void DrawDialogChrome(Dialog dialog, TerminalCell[,] cells, Size size, int width, int height)
    {
        ...
        var y = dialog.RealDim.Location.Y;
        ...
        var titleStartX = x + 2;
        var titleMaxWidth = width >= 20
            ? width - 18
            : width - 4;
        PutText(cells, size, titleStartX, y, dialog.Text ?? string.Empty, Math.Max(0, titleMaxWidth), foreground, background);
        ...
        if (width >= 7)
        {
            ...
            PutText(cells, size, buttonX + buttons.Length - 2, y, "X", 1, ConsoleColor.Red, background);
        }
    }
    ...
    {
        foreach (var child in control.Children.Reverse<IControl>())
        {
            // canvas position of this child
            int canvasCol = child.RealDim.Location.X;
            int canvasRow = child.RealDim.Location.Y;
            if (child.IsFixed)
            {
                // Fixed children don't scroll — draw at absolute position
                DrawControl(child, cells, size);
            }
            else if (control is IGroupControl)
            {
                // Clip against visible canvas area only
                if (canvasCol + child.RealDim.Width <= 0 || canvasCol >= size.Width)
                    continue;
                if (canvasRow + child.RealDim.Height <= 0 || canvasRow >= size.Height)
                    continue;
                DrawControl(child, cells, size);
            }
            else
            {
                /* Non-scroll container children: clip to parent bounds. */
                var parentRect = new Rectangle(
                    control.RealDim.Location,
                    new Size(Math.Max(1, control.size.Width), Math.Max(1, control.size.Height)));
                var childRect = new Rectangle(
                    child.RealDim.Location,
                    new Size(Math.Max(1, child.size.Width), Math.Max(1, child.size.Height)));
                if (!childRect.IntersectsWith(parentRect))
                    continue;
                var requiresClipping = !parentRect.Contains(childRect);
                if (!requiresClipping)
                {
                    DrawControl(child, cells, size);
                    continue;
                }
                var minX = Math.Max(0, childRect.Left);
                var minY = Math.Max(0, childRect.Top);
                var maxX = Math.Min(size.Width - 1, childRect.Right - 1);
                var maxY = Math.Min(size.Height - 1, childRect.Bottom - 1);
                if (minX > maxX || minY > maxY)
                    continue;
                var backup = new TerminalCell[maxX - minX + 1, maxY - minY + 1];
                for (var row = minY; row <= maxY; row++)
                    for (var column = minX; column <= maxX; column++)
                        backup[column - minX, row - minY] = cells[column, row];
                DrawControl(child, cells, size);
                for (var row = minY; row <= maxY; row++)
                    for (var column = minX; column <= maxX; column++)
                        if (!parentRect.Contains(column, row))
                            cells[column, row] = backup[column - minX, row - minY];
            }
        }
    }
    ...
    private static void DrawBorder(IControl control, TerminalCell[,] cells, Size size, int width, int height)
    {
        ...
        var glyphs = definition.Style switch
        {
            BorderStyle.Double => new[] { '═', '║', '╔', '╗', '╚', '╝' },
            BorderStyle.Simple => new[] { '-', '|', '+', '+', '+', '+' },
            BorderStyle.Raised => new[] { '▀', '▌', '▛', '▌', '▀', ' ' },
            BorderStyle.Lowered => new[] { '▄', '▐',' ', '▄', '▐', '▟' },
            _ => new[] { '─', '│', '┌', '┐', '└', '┘' }
        };
        for (var column = 0; column < width; column++)
        {
