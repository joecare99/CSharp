...
Define how archive-import planning and execution state is persisted in runtime-defined storage so interrupted migrations can be resumed reliably, including on another machine where supported.
...
The persisted state should live in a runtime-defined storage area rather than only in transient application state. That allows an interrupted import to continue later on the same machine and, when the storage root and source location are both available, on another machine that can access equivalent persisted state.
...
The reviewed persistence direction should also remain compatible with the provider-agnostic structured-change model. The first archive slice remains snapshot-oriented, but later slices may persist mixed plans that include patch-derived change sets, explicit path rewrites, and compatibility-adapter progress before native structured-change sinks exist.

## Scope

- Define a durable archive-import plan manifest stored under runtime-defined provider storage
...
- Clarify how later structured-change and compatibility-adapter progress can extend the same persistence model

## Acceptance Criteria

- Archive-import planning and execution state is represented through durable runtime artifacts
...
- The reviewed model leaves room for later mixed plans, path rewrites, and compatibility-adapter checkpoints

## Assumptions

- Runtime-defined storage is available to the application independent of whether a repository-local `DevOps` folder exists
...
- Should persisted status live under application data, a workspace-local runtime folder, or a provider-configured storage root?
