...
public sealed class PipelinedMigrationProgramTests
{
    ...
    {
        var sMissingWorkingCopy = Path.Combine(Path.GetTempPath(), "RepoMigrator.Tools.PipelinedMigration.Tests", Guid.NewGuid().ToString("N"), "missing-svn-wc");
        var sTargetPath = Path.Combine(Path.GetTempPath(), "RepoMigrator.Tools.PipelinedMigration.Tests", Guid.NewGuid().ToString("N"), "git-target");
