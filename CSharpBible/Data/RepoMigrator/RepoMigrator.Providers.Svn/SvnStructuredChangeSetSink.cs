...
/// <summary>
/// Materializes normalized structured changes into a working directory and delegates commits to the existing Subversion target provider.
/// </summary>
public sealed class SvnStructuredChangeSetSink : IMigrationChangeSetSink
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="SvnStructuredChangeSetSink"/> class.
    /// </summary>
    public SvnStructuredChangeSetSink(IVersionControlProvider versionControlProvider, string? workspaceRootPath = null)
    {
    ...
    /// <inheritdoc />
    public string Name => $"Structured SVN sink ({_versionControlProvider.Name})";

    /// <inheritdoc />
    public bool CanHandle(MigrationDestinationDefinition destination)
        => destination is not null
           && destination.Kind == MigrationDestinationKind.Repository
           && string.Equals(destination.Repository?.ProviderKey, SvnCliProvider.ProviderKey, StringComparison.OrdinalIgnoreCase);
    ...
    public Task<ChangeApplicationCapabilities> GetCapabilitiesAsync(MigrationDestinationDefinition destination, CancellationToken ct)
    {
        ...
        if (!CanHandle(destination))
            throw new NotSupportedException("The supplied destination definition is not a Subversion-backed repository destination.");
    ...
    public async Task ApplyChangeSetAsync(MigrationChangeSet changeSet, IMigrationProgress progress, CancellationToken ct)
    {
        ...
        await _versionControlProvider.CommitSnapshotAsync(
            ...
            new CommitMetadata
            {
                ...
                TargetBranch = _endpoint!.BranchOrTrunk,
        ...
        foreach (var directoryChange in directoryChanges)
        {
            switch (directoryChange.Kind)
            {
                ...
                case MigrationDirectoryChangeKind.Copy:
                    ...
                    break;
                default:
                    throw new NotSupportedException($"The directory change kind '{directoryChange.Kind}' is not supported by the SVN structured sink.");
            }
        }
    }
        ...
        foreach (var fileChange in fileChanges)
        {
            switch (fileChange.Kind)
            {
                ...
                case MigrationFileChangeKind.ModeChange:
                    ...
                    break;
                default:
                    throw new NotSupportedException($"The file change kind '{fileChange.Kind}' is not supported by the SVN structured sink.");
            }
        }
    }
    ...
    private void WriteFile(string relativePath, MigrationFileChange fileChange)
    {
        ...
        ApplyModeMetadata(fileChange, absolutePath);
    }
    ...
    private byte[] ResolveBinaryPayload(MigrationBinaryChange binaryChange)
    {
        return binaryChange.PayloadMode switch
        {
            ...
            _ => throw new NotSupportedException($"The binary payload mode '{binaryChange.PayloadMode}' is not supported by the SVN structured sink.")
        };
    }
    ...
    private byte[] ReadBinaryPayloadFromReference(BinaryPayloadReference? payloadReference)
    {
        ...
        if (payloadReference.Kind != BinaryPayloadReferenceKind.RelativeArtifactPath)
            throw new NotSupportedException($"The binary payload reference kind '{payloadReference.Kind}' is not supported by the SVN structured sink.");
    ...
    private void EnsureInitialized()
    {
        if (_endpoint is null)
            throw new InvalidOperationException("The structured SVN sink has not been initialized.");
    }
}
