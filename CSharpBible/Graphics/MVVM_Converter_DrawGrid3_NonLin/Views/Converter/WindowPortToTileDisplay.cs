...
namespace MVVM_Converter_DrawGrid3_NonLin.Views.Converter
{
    /// <summary>
                ...
                case SWindowPort c when parameter is string s:
                    ...
                    for (var i = 0; i < c.tiles.Length; i++)
                    {
                        FrameworkElement l = new System.Windows.Shapes.Rectangle()
                        {
                            ...
                            RenderTransform = new TranslateTransform((c.tiles[i].position.X*2 + (c.tiles[i].position.Y % 2)) * (dSize.Width /2), c.tiles[i].position.Y * dSize.Height),
