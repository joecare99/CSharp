...
public static class Program
{
    ...
    private static void PlayProceduralMusic()
    {
        ...
        while (_isPlaying)
        {
            ...
            if (step % 8 == 0)
            {
                int bassNote = chords[rnd.Next(chords.Length)];
                // Play Bass Note on Channel 0
                SendMidiMsg(0x90, bassNote, 80, 0);
                // Cut the bass note shortly before next cycle
                Task.Delay(1400).ContinueWith(_ => SendMidiMsg(0x80, bassNote, 0, 0));
            }

            // Prozedurale Melodie-Generierung: Wähle harmonische Töne basierend auf Pentatonik
            int melodyNote1 = scale[rnd.Next(scale.Length)];

            // Arpeggio-Effekt: Manchmal eine zweite komplementäre Note spielen
            int melodyNote2 = melodyNote1 + 4; // Terz / Intervall-Shift
