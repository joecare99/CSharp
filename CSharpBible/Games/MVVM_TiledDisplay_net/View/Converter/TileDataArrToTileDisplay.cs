...
{
    /// <summary>
    /// Class TileDataArrToTileDisplay.
    /// Implements the <see cref="IValueConverter" />
    /// </summary>
    /// <seealso cref="IValueConverter" />
    public class TileDataArrToTileDisplay : IValueConverter
    {
        /// <summary>
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            switch (value)
            {
                case TileData[] tiles when parameter is string s:
                    ...
                    if (brushes.Count > 0)
                        for (var i = 0; i < tiles.Length; i++)
                        {
                            ...
                            FrameworkElement l = new System.Windows.Shapes.Rectangle()
                            {
                                ...
                                RenderTransform = new TranslateTransform(tiles[i].position.X * dSize.Width, tiles[i].position.Y * dSize.Height),
                                Fill = brushes[tiles[i].tileType < brushes.Count ? tiles[i].tileType : 0],
                                Style = ItemStyle
                            };
                            result.Add(l);
                        }
