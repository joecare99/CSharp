...
namespace WinAhnenCls.Model.GenBase.Tests
{
    ...
    public class GedComBaseReaderTests
    {
        ...
        private void AssertAreEqual(object v, GedComBaseReader.SGedComLine sLine, string msg = "")
        {
            ...
            if (_ao.Length >= 4)
                Assert.AreEqual(_ao[3], sLine.sData, $"{msg}.{nameof(sLine.sData)}");
            else
                Assert.IsNull(sLine.sData, $"{msg}.{nameof(sLine.sData)}");
            if (_ao.Length >= 5)
                Assert.AreEqual(_ao[4], sLine.sXRef, $"{msg}.{nameof(sLine.sXRef)}");
            else
                Assert.IsNull(sLine.sXRef, $"{msg}.{nameof(sLine.sXRef)}");
            if (_ao.Length >= 6)
                Assert.AreEqual(_ao[5], sLine.sRest, $"{msg}.{nameof(sLine.sRest)}");
            else
                Assert.IsNull(sLine.sRest, $"{msg}.{nameof(sLine.sRest)}");
        }
    }
}