...
namespace WinAhnenCls.Model.HejInd
{
    ...
    private Dictionary<EHejIndDataFields, object> _data = new();
        ...
        public class _HejIndiDataProxy
        {
            ...
            private CHejIndiReader This { get; }
            public _HejIndiDataProxy(CHejIndiReader This)
            {
                this.This = This;
            }
        }
        ...
        public IEnumerable<IGenPerson> Read(StreamReader sr)
        {
            while (!sr.EndOfStream && (char)(sr.Peek()) != 'm') // 'mrt
            {
                ...
                yield return this as IGenPerson;
            }
        }

        public IEnumerable<IGenPerson> Read(string path)
        {
            using var sr = new StreamReader(path);
            return Read(sr);
        }

        public IEnumerable<IGenPerson> Read(Stream stream)
        {
